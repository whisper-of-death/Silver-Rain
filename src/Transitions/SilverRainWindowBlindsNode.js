...
//               All rights reserved
// ----------------------------------------------
//          SilverRainWindowBlindsNode
// ----------------------------------------------

import {SilverRainTransitionNode} from './../SilverRainTransitionNode.js';

class SilverRainWindowBlindsNode extends SilverRainTransitionNode {
    ...
    __createFragmentShaderSource() {
        this.__fSrc = `
			...
			void main() {
				...
				float t = u_mix;
				if (mod(floor(v_texCoord.y*100.*u_mix),2.)==0.)
					t*=2.-.5;
				gl_FragColor = mix(
					color1,
					color2,
					mix(t, u_mix, smoothstep(0.8, 1.0, u_mix))
				);
			}
        `;
	}
...
export {SilverRainWindowBlindsNode};
