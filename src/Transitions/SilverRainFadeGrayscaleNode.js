...
//               All rights reserved
// ----------------------------------------------
//          SilverRainFadeGrayscaleNode
// ----------------------------------------------

import {SilverRainTransitionNode} from './../SilverRainTransitionNode.js';

class SilverRainFadeGrayscaleNode extends SilverRainTransitionNode {
	// Input
	intensity = 0.3;
    ...
    __createFragmentShaderSource() {
        this.__fSrc = `
			...
			void main() {
				...
				gl_FragColor = mix(
					mix(vec4(grayscale(color1.rgb), 1.0), color1, smoothstep(1.0-u_intensity, 0.0, u_mix)),
					mix(vec4(grayscale(color2.rgb), 1.0), color2, smoothstep(u_intensity, 1.0, u_mix)),
				u_mix);
			}
        `;
	}
...
export {SilverRainFadeGrayscaleNode};
